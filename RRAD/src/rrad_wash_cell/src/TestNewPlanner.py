...
###########################
#Note: Remove xyavg from binning
...
eps = 0.05  #epsiolo value for determining angle threshold
...
def run_gui(_mesh):
    ...
    def callback():
        global sprayer_width,Rot_Deg,R_axis,Adaptive,offset,sprayer_distance, sprayer_adjust, bintype, speed, sprayer_angle, PathPlan
        R_axis = r_axis.get()
        Adaptive = adapt.get()
        bintype = binmethod.get()
        PathPlan = planner.get()
        Rot_Deg = 0
        sprayer_distance = 11
        offset_percent = 10
        speed = 10
    ...
    Label(master, text="Plan for Whole Part:").grid(row=8)
    ...
    b15 = Radiobutton(master, text="Yes", variable=planner, value=1)
    b16 = Radiobutton(master, text="Skip to Partial Part Selection", variable=planner, value=2)
    ...
    b15.grid(row=8, column=1,sticky=W)
    b16.grid(row=8, column=2,columnspan=3, sticky=W)
...
def fit_to_side(_points, _mesh,maxz,minz,sprayer_distance,sprayer_width,orig_length):
    ...
    for h in range(1,(hsteps+1)):
        zh = first_slice - offset*h
        ...
        end_of_slice = modpath[-1]
        dtime, t_analysis,slice_dist = timestamper(modpath,binpoints,polar,lastpoint)
...
def export2matlab(matexport):
    for x in range(len(matexport)):
        ...
        filename = HOME+'/RRAD/src/rrad_wash_cell/src/Test_Data/slice'+ str(matexport[x][2]) +'height' + str(round(matexport[x][3],2)) + '.csv'
...
#######################################################################
    ...
    #def intpol_between(_mesh, zheight, intersect):     #combined into one function
    #facets = len(_mesh.normals)
    newsurf = []
    origsurf = []
    checksurf = []
...
def intpol(_mesh, zheight, intersect):
    ...
    newsurf = path_prep(origsurf,newsurf, zheight)

    sortsurf, pol_cords = polar_sort(newsurf)

    binsurf, pol_cord = create_bins(sortsurf,pol_cords)######path resizing   add min
...
def combine_export(_line,_path, times):
    ...
    print 'excomb',len(excomb),( p,'p',l,'l',t,'t')
    for x in range(0,p):
        k = x + 1
...
##########
    ...
    if PathPlan == 2:
        analysis = [] 
        for i in range(len(orig_mesh.normals)):
        #path is stil invalid so godot crashes
            analysis.append([i,0])
        comb = [[0,2*maxx,2*maxy,2*maxz,0,0,0,2*maxx+1,2*maxy+1,2*maxz+1,2,2*maxx-1,2*maxy-1,2*maxz-1,0],[0,2*maxx,2*maxy,2*maxz,1,1,1,2*maxx+1,2*maxy+1,2*maxz+1,2,2*maxx-1,2*maxy-1,2*maxz-1,0],[1,2*maxx+2,2*maxy+2,2*maxz+2,1,1,1,2*maxx+3,2*maxy+3,2*maxz+3,2,2*maxx+1,2*maxy+1,2*maxz+1,1],[2,2*maxx+4,2*maxy+4,2*maxz+4,1,1,1,2*maxx+5,2*maxy+5,2*maxz+5,2,2*maxx-3,2*maxy-3,2*maxz-3,2]]
        built_path = path_to_msg(comb)
        impingement = analysis_to_msg(analysis)
        print( 'No Original Path Plan')
        #print fitpath
